...
class SudokuBoard:
    ...
    def computeConstraintsSets(self):
        ...
        # Major Block -> sudoku board has nine, further divided into nine squares
        # Move to a major block, then create a tuple for each entry in that block
    ...
    def computePointDict(self):
        ...
        def relevant(p): 
            return filter((lambda x: p in x), self.constraints)
    ...
    #Determines if all constraints are filled in
    #If all constraints are filled, then the game is complete
    def isSolved(self):
        ...
        return reduce(lambda x,y: x and y, map(satisfied, self.constraints), True)
